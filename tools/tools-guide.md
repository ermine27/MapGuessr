...
国を表す2文字の英字コード（`JP`、`US`、`DE` など）で国を指定します。国コードの一覧は `list countries` で確認できます。  
出力ファイルは省略時 `tools/output/areas/jp.geojson` のようになります。

#### `state` — 州・都道府県を抽出

```bash
# 国コード+地域コードで1件だけ抽出（例: US.CA = アメリカのカリフォルニア州）
...
国コードや地域コードが分からないときに調べるために使います。`country` モードの国コード、`state` モードの地域コード、`continent` モードの大陸名を確認する際の参照として利用します。

```bash
node tools/extract-area.js list countries        # 全国の一覧（国コード付き）
...
実行中はコンソールに以下の形式で進捗が表示されます。

```
[  423 / 1000] 42.3%  試行: 1,847  成功率: 22.9%  経過: 0:03:12  残り予測: 0:04:17
```

| 項目 | 説明 |
|------|------|
| `423 / 1000` | 収集済み件数 / 目標件数 |
| `42.3%` | 進捗率 |
| `試行` | APIリクエスト総数（Street View が見つからなかった地点を含む） |
| `成功率` | 試行のうちロケーションとして採用できた割合 |
| `経過` | 開始からの経過時間（`H:MM:SS`） |
| `残り予測` | 完了までの推定時間（`H:MM:SS`） |
...
```
file:///c:/{ソースコードを置いたパス}/MapGuessr/tools/preview-area.html
```

ローカルサーバーを立てている場合は、サーバーの URL で開いてください（例: VS Code の Live Server 拡張、Python の `http.server` など）。

```
http://localhost:5500/tools/preview-area.html
http://localhost:8000/tools/preview-area.html
```
...
```
file:///c:/{ソースコードを置いたパス}/MapGuessr/tools/preview-map.html
```

ローカルサーバーを立てている場合は、サーバーの URL で開いてください（例: VS Code の Live Server 拡張、Python の `http.server` など）。

```
http://localhost:5500/tools/preview-map.html
http://localhost:8000/tools/preview-map.html
```
