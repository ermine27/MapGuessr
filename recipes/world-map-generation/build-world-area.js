 ...
 * Natural Earth GeoJSON から該当国を抽出して world.geojson を生成する。
 *
 * 使い方:
 *   node tools/build-world-area.js
 */
...
const outputPath = path.join(__dirname, "output", "areas", "world.geojson");
...
for (const [code, name] of nameFallbacks) {
    ...
    if (feature) {
        ...
        props.iso_a2 = code;          // -99 を正しいコードで上書き
...
// ── 結果レポート ──
if (remaining.size > 0) {
    console.warn(`⚠ マッチしなかったコード (${remaining.size} 件):`);
    for (const code of remaining) {
        console.warn(`  - ${code}`);
    }
}
console.log(`マッチした Feature: ${matched.length} 件`);
...
console.log(`出力: ${outputPath} (${sizeMB} MB)`);
